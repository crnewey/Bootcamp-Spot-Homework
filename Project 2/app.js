function buildMetadata(Final_df) {
  var metadataURL = "/metadata/" + Final_df;
...
function buildCharts(Final_df) {
  var chartsURL = "/final_df/" + Final_df;
  d3.json(chartsURL).then((data) => {
    var trace1 = {
      x: data.otu_ids,
      y: data.Final_df_values,
      mode: 'markers',
      text: data.otu_labels,
      marker: {
        color: data.otu_ids,
        size: data.Final_df_values,
        colorscale: "Earth"
      }
    };
    var trace1 = [trace1];
    ...
    var trace2 = [{
      values: data.Final_df_values.slice(0,10),
      labels: data.otu_ids.slice(0,10),
      hovertext: data.Year.slice(0,10),
      type: "pie",
      marker: {
        colorscale: "Earth"
      }
